...
import { findMultiRoute } from "../src";
...
describe('Parallel ConstuntProduct Combo Pool', () => {
  it('One pool', () => {
    ...
    for (let i = 0; i < 10; ++i) {
      ...
      const out2 = findMultiRoute(token0, token1, amountIn, [pool], token1, gasPrice).amountOut
    ...
    for (let i = 0; i < 10; ++i) {
      ...
      const out2 = findMultiRoute(token1, token0, amountIn, [pool], token1, gasPrice).amountOut
      expect(Math.abs(out1/out2-1)).toBeLessThan(1e-12)
    }
  ...
  it('Several equal pools', () => {
    ...
    for (let i = 0; i < 7; ++i) {
      ...
      for (let i = 0; i < 100; ++i) {
        ...
        const {totalAmountOut} = findMultiRoute(token0, token1, amountIn, pools, token1, gasPrice)
